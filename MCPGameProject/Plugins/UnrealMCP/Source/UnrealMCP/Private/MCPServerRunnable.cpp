    ...
    while (bRunning)
    {
        ...
        if (ListenerSocket->HasPendingConnection(bPending) && bPending)
        {
            ...
            if (ClientSocket.IsValid())
            {
                ...
                int32 SocketMCPServerBufferSize = 65536;  // 64KB buffer
                ClientSocket->SetSendBufferSize(SocketMCPServerBufferSize, SocketMCPServerBufferSize);
                ClientSocket->SetReceiveBufferSize(SocketMCPServerBufferSize, SocketMCPServerBufferSize);
...
void FMCPServerRunnable::HandleClientConnection(TSharedPtr<FSocket> InClientSocket)
{
    ...
    const int32 MaxMCPServerBufferSize = 4096;
    uint8 Buffer[MaxMCPServerBufferSize];
    ...
    while (bRunning && InClientSocket.IsValid())
    {
        ...
        bReadSuccess = InClientSocket->Recv(Buffer, MaxMCPServerBufferSize, BytesRead, ESocketReceiveFlags::None);
